          ...
          return (
            <TableRow key={t.id} className={`${isDebit || amount[0] === '-' ? 'bg-[#fffbfa]' : 'bg-[#f6fef9]'} !over:bg-none !border-b-DEFAULT`}>
              ...
              <TableCell className={`pl-2 pr-10 font-semibold ${
                ...
                ? 'text-[#f04438]' 
                : 'text-[#039855]'
                }`}>{isDebit ? `-${amount}` : amount}</TableCell>
              <TableCell className='pl-2 pr-10'>
                <CategoryBadge category={status} />
                </TableCell>
              <TableCell className='min-w-32 pl-2 pr-10'>{formatDateTime(new Date(t.date)).dateTime}</TableCell>
              <TableCell className='min-w-24 pl-2 pr-10'>{t.paymentChannel}</TableCell>
              <TableCell className='max-md:hidden pl-2 pr-10'><CategoryBadge category={t.category} /></TableCell>
            </TableRow>
          );
        })}
      </TableBody>
